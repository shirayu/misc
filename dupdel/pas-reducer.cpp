...
int main(int argc, char *argv[]){
    ...
    if(argc != 4){
        std::cerr << "[Usage] " << argv[0] << " input_folder output_folder mode[0,1]" << std::endl;
        return -1;
    };
    ...
    for(std::vector<std::string>::iterator it = fnames.begin(); it!=fnames.end(); ++it){
        ...
        for(size_t pos = 0 ; pos < total; ++pos){
            if(buf[pos] == '\n'){
                if ( (mode == 0 and buf[prev]  =='+')
                        or (mode==1 and buf[prev] == '*')
                        ){
                    std::string s(buf + prev+2 , buf + pos);
                    ++mymap[s];
                }
                prev = pos + 1;
            };
    ...
    std::ofstream* ofs = new std::ofstream(out_fname.c_str());
    ...
    for(MYMAP::iterator it = mymap.begin(); it != mymap.end(); ++it){
        *ofs << it->second << "\t" << it->first << std::endl;
        ...
        if(count == ONE_FILE_LINE_NUM){
            ...
            ofs->close();
            ofs = new std::ofstream(out_fname.c_str());
        };
    };
    ofs->close();
